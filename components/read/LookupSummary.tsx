...
export default function LookupSummary({ totalVocab, distinctWords, clozeAnswered }: Props) {
  ...
  const msg = totalVocab === 0
    ...
    : clozeAnswered === 0
      ? (<><strong style={{ color: 'var(--ink)' }}>{totalVocab} blank{totalVocab === 1 ? '' : 's'}</strong>{repeats && <> covering <strong style={{ color: 'var(--ink)' }}>{distinctWords} review word{distinctWords === 1 ? '' : 's'}</strong></>} — type each one to practice. <strong style={{ color: 'var(--ink)' }}>Hints</strong> above shows the English meaning when you hover a blank; turn it off to recall each word cold.</>)
      : remaining > 0
