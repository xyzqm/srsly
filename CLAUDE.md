...
One structural consequence: **A1 is frozen.** Promotion into band `b` needs `b - anchor >= 2`, which for `b = 1` would need an anchor below A1, and there is no such level — so the A1↔A2 boundary never trades and the 500 most cross-register-frequent words stay exactly as frequency ranked them. That is the right outcome. The anchor's resolution is not fine enough to second-guess the core: at a one-level threshold it demoted `algo`, `bueno` and `trabajar` out of A1. In practice the cross-check operates from A2↔B1 upward, moving 200–270 words per language (~2%).

**Hand-pinned core vocabulary.** `scripts/data/core-overrides.json` lists headwords forced to level 1, applied by `scripts/lib/coreOverrides.mjs` *after* the anchor and immediately before emission, so pinning bypasses both signals. It exists because frequency and the anchor share a blind spot: **nobody writes "hello" in an encyclopedia.** Against the corpora actually used, `hola` and `bonjour` landed at B1, `안녕하세요` and `아니요` at B2, and `por favor` / `s'il vous plaît` / `au revoir` / `여보세요` were never ranked at all, since the corpus tokenizer counts single tokens and those are multi-word. No tuning fixes that — the evidence is absent from the source.
