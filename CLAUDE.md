...
**French only, because the data is.** Lexique 3 is already vendored for frequency ranking and
its `infover`/`genre`/`nombre` columns record every form's exact slot. Spanish would need the
tags `build-esdict.mjs` currently reads and discards; Japanese has kuromoji at runtime; Chinese
has no inflection to describe. The seam is `lib/frenchGrammar.ts`, so a second language is a
second table rather than a second design.
...
units, grammar and vocabulary interleaved. It is French only, hidden entirely elsewhere —
`TabNav` filters on `hasLessons`, because an empty tab reads as a broken one.
