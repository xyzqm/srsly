...
- **Prefer a well-maintained library to hand-rolled logic — but say so when you don't.** The
  ...
  `Card`. Kuromoji, JSZip, hanzi-writer and the Supabase client are all used rather than
  reimplemented. `wanakana` was added for romaji→kana and then REMOVED: typed recall now uses
  the learner's own OS IME, and a library rewriting the same input the IME is rewriting fights
  it for every character. A dependency earns its place by what it does for the learner, not by
  being well-made.
...
| `build-strokes.mjs` | `public/strokes/*.json` — stroke outlines for the 2,663 HSK characters, subset from `hanzi-writer-data` under the Arphic Public License |

**`scripts/sweep-readability.mjs` is not a build script — it MEASURES.** It drives the running
app over the DevTools Protocol, generates N passages per level and dumps the day's cache, which
is what produced the numbers in `lib/passageTheme.ts` and `lib/spanishForms.ts`. It drives the
APP rather than `/api/daily-content` on purpose: the route answers in the RawTok wire format and
`useDailyContent` normalises it, so measuring the raw tuples would measure a different number
from the one on screen — and the app disagreeing with itself is the thing these sweeps exist to
catch. Its header records the four traps that cost an afternoon (finishing is not filling, the
...
cloze progress, the activity log, finished lessons and handwriting progress all sync. Staying device-local, each for
...
other word the learner owns that contains it — so keyed by word, one character's schedule would
exist in as many copies as it has words and they would disagree from the first review. Keyed by
character it is also a SMALLER set than the deck rather than a doubling of it: HSK's 4,991 words
are built from 2,663 distinct characters.
...
daily requirement — so being a month behind on it must never read as a broken reading streak.
`dueWritingChars` is a separate function rather than a flag on the existing one so no reading
surface can start counting it by accident, and `tests/writingState.test.ts` asserts that none of
the seven reading modules imports this one. That test strips comments before checking, because
this module's prose names `isDueToday` repeatedly to explain what it is firewalled FROM — a raw
substring check would fail on the documentation and push the next person to delete the
explanation to get CI green.

**A card is owned WHOLE on merge, not merged per field.** Stability, difficulty, lapses and due
date describe one review history, so a per-field maximum composes a state neither device was
ever in — the same argument `lib/srsStateMerge.ts` makes about the streak. The later
`lastReview` takes the card. Ties fall through reviews, lapses and stability to canonical JSON
order, because the rule must be COMMUTATIVE and not merely deterministic: a device writes its
...
flashcard knows, so the learner is not asked: clean is Good, one or two wrong is Hard, three or
a hint is Again. Easy means "I knew this instantly and want a much longer interval" and a stroke
count cannot observe it — offering it would invent a distinction the input cannot support.

**`fsrsSchedule` now takes a `Schedulable`, not a `DeckWord`.** It only ever read the scheduling
fields; requiring a whole word meant a character card would have had to invent a hanzi, a pinyin
and a gloss to satisfy a type, which is how a fake value gets stored. A `DeckWord` satisfies the
narrower type structurally, so no existing caller changed.

**CHINESE ONLY, and the reason is data honesty rather than scope.** Measured against
`hanzi-writer-data`, 88.7% of JLPT kanji are present — but the 223 missing are exactly the
shinjitai (厳 倹 権 鉱 渉 沢 団 読 売 楽 児 発), and among those that ARE present are characters
whose Japanese stroke order or shape differs (直, 令, 骨). It would teach confidently wrong
stroke order for a subset nobody could identify, which is worse than teaching none.
`hanzi-writer-data-jp` is not the answer either: it is marked experimental and its npm package
contains no data files at all. Japanese needs KanjiVG and its own build script.

**ONE FILE PER CHARACTER, which breaks this file's usual rule on purpose.** Every other dataset
is a single JSON fetched whole, because lookups are arbitrary. Stroke data is the opposite: a
session practises a handful of characters. Measured at ~2.4 kB each, a 20-character session
costs ~48 kB, against 1.41 MB for even a level-3 shard and 7.00 MB for the lot — and it is the
shape `HanziWriter`'s own `charDataLoader` expects. The cost is 2,663 files in the repository,
which is the honest trade: identical bytes, worse file count.

**The licence is an obligation, not a footnote.** The Arphic Public License requires
`ARPHICPL.TXT` in every copy (§1) and, because subsetting DELETES characters from the glyph
table, a statement of what changed and when (§2a) plus the modification being freely available
...
is the field. A toggle on the existing flashcard — not a fifth tab, not a new card type,
`gradeCard` untouched, no schema change.

**MEANING IN, WORD OUT, IN ALL FOUR LANGUAGES.** The card shows the English meaning; the learner
types the word. Chinese and Japanese are typed with the learner's own OS IME, so the answer is
the CHARACTERS — 朋友, 食べる — exactly as Spanish and French have always asked for `comer` and
`parler`.

**IT USED TO FACE zh/ja THE OTHER WAY, and the old argument is kept because it was not wrong.**
The first design showed the character and asked for the READING, reasoning that an IME hands
you the character once you have the sound. That is TRUE and was measured: a multi-character HSK
word has a median of ONE homophone in the whole of CC-CEDICT, so pinyin → characters is close
to deterministic. What it got wrong is that this is not the only thing worth testing —
producing a word from its meaning and picking the right characters is what writing Chinese on a
keyboard actually IS, and a learner asking to practise that is asking for a real skill. Changed
on request (2026-09-07) after testing the live build.

**THE SYNONYM PROBLEM IS ANSWERED BY THE BUTTONS THAT WERE ALREADY THERE.** "Give the word for
friend" has more than one right answer and the deck stores one, so the grader will sometimes
mark a valid synonym wrong. The old design met that with a bespoke "I was actually right"
override; it is gone. Again/Hard/Good/Easy are on screen the moment the answer is revealed, and
pressing Good does precisely what the override did. A second control for one action is one more
thing to explain and one more path to the same grade.

**THE `close` TIER IS NOW THE FORGOTTEN-KEYBOARD TIER**, and it is the reason the pinyin and
kana work survived the pivot. Type `pengyou` with the IME switched off, or press Enter on
たべる before converting it to 食べる, and you have shown you know the word and failed only to
produce the script. That grades Good and shows the correct form rather than recording a lapse.
The comparison is deliberately toneless for Chinese, because toneless pinyin is what an IME is
actually driven with. For Japanese the realistic miss is NOT romaji — with an IME on you never
see romaji — it is unconverted kana, which needs no romaji table at all.

A katakana loanword typed in hiragana lands in the same tier: こーひー is not an alternative
spelling of コーヒー, it is the reading in the wrong script.

**`wanakana` WAS REMOVED, AND THAT IS THE POINT OF THE PIVOT.** It converted romaji to kana by
rewriting the input's value as the learner typed. With a real Japanese keyboard that is two
things transforming the same keystrokes, and they fight over every character. The app now keeps
its hands off the field entirely — no library, no binding, and `lib/typedAnswer.ts` stays
synchronous because every comparison left is a codepoint operation.

**`isComposing` IS LOAD-BEARING, NOT DEFENSIVE.** Enter during composition COMMITS the IME's
candidate — 朋友 out of `pengyou` — and must not also submit the card. Without that guard the
first Enter of every Chinese and Japanese answer would grade a half-finished string. The input
also stays UNCONTROLLED: a controlled React input re-renders mid-composition and can drop the
candidate window or jump the caret, so the element owns its value and `submit()` reads the ref.

**WHAT IS DELIBERATELY NOT FOLDED**: `ü` and `ñ` are LETTERS, not accents — folding them merges
女 nü with 努 nu and accepts `ano` for `año`. `ç` IS folded, because no French pair turns on a
cedilla. The test is whether the mark distinguishes two words, not how it is drawn. NFKC is
applied to the CJK comparison so a full-width `ａ` from a CJK keyboard in the wrong mode is the
same answer as `a`.

**Audio is hidden until reveal**, as in reverse mode: the play button speaks the word, and the
word is the answer.
