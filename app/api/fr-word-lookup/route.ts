...
/**
 * Resolves a single manually-typed French word to its dictionary (base) form, so a
 * conjugated word typed into AddWordForm (mangé → manger) is stored under the same
...
export async function POST(req: NextRequest) {
  ...
  const r = await resolveWordServer('fr', text);
